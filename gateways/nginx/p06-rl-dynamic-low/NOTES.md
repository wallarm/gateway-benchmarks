# `nginx / p06-rl-dynamic-low` — compliance notes
...
| `10 IPs × 15 rps × 3 s → ≥ 120 (±30) × 429` | `limit_req_zone $http_x_real_ip zone=bench_p06:1m rate=10r/s;` + `limit_req zone=bench_p06 burst=10 nodelay;` |

Observed `341 × 429` is far above the `120 − 30 = 90` minimum
threshold, so the cell has comfortable headroom even on cold Docker
Desktop / CI runners.

Cross-gateway symmetry with `wallarm/p05` (`99 × 2xx + 351 × 429`):
...
server {
    location / {
        limit_req zone=bench_p06 burst=10 nodelay;
...
* **`burst=10 nodelay`.** Absorbs a one-second burst per IP without
  ...
  inside ~1 request.
* **`zone=bench_p06:1m`.** 1 MB ≈ >= 8 000 keys — two orders of
  magnitude over p05's 100-IP pool. We keep the floor at 1 MB to
  make the config diffable against p03 (same zone size, different
  key) and p06 (same key, ten times the zone).

## Uniform-settings audit

Same ten rows as
[`p01-vanilla/NOTES.md`](../p01-vanilla/NOTES.md#uniform-settings-audit)
— the `http {}` block is identical between p01, p03 and p05 except
