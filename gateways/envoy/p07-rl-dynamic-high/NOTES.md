# `envoy / p07-rl-dynamic-high` — compliance notes

**Current verdict on `envoyproxy/envoy:distroless-v1.32.6`**: `PASS (3/3)`
with an enumerated-descriptors deviation (shared with p05; see
§ Deviation below).
...
Identical to p05, different per-IP bucket size. Three pieces in
...
**Canonical pool**: 50 000 distinct IPs per §p06.
**Actual**: 11 enumerated descriptors
(`10.5.0.1..10.5.0.10 + 10.5.9.9`). Same root cause as p05 —
...
See `docs/GATEWAYS.md § Deviations → [gw=envoy, p=p06-rl-dynamic-low / p07-rl-dynamic-high, infra=enumerated-descriptors]`
for the full post-mortem (shared with p05).

## Why no standalone "burst produces 429" in `setup.sh`

Same rationale as p03 / p05: envoy bootstraps entirely from
