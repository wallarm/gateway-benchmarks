#!/usr/bin/env bash
# gateways/envoy/p07-rl-dynamic-high/setup.sh
#
...
# per-IP descriptor path cleanly before the burst probes hammer the
# buckets.
#
# Environment:
...
# -----------------------------------------------------------------------------
say "envoy/p07-rl-dynamic-high: waiting for ${DATA_URL}"
for _ in $(seq 1 30); do
    if curl -sS -o /dev/null --max-time 2 \
            -H 'X-Real-IP: 10.5.0.1' "${DATA_URL}/status/200" \
         2>/dev/null; then
