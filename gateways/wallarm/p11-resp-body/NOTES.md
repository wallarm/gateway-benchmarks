# wallarm / p11-resp-body — notes

Canonical policy — [`docs/POLICIES.md` §
p10](../../../docs/POLICIES.md):
...
Same vehicle as p09 — `lua_runner` + `cjson.safe` — but on
...
Against the Wallarm API Gateway (native arch — see qemu gotcha
in [`p08-req-headers/NOTES.md`](../p08-req-headers/NOTES.md)):

```
==> parity: gateway=wallarm profile=p11-resp-body target=http://localhost:9080
