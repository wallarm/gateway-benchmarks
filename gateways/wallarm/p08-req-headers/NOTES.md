# wallarm / p08-req-headers — notes

Canonical policy — [`docs/POLICIES.md` §
p07](../../../docs/POLICIES.md):
...
`the Wallarm gateway` strips `base_path` and prepends `target.endpoint.url`,
...
we cannot point `bench-p08-headers` directly at those endpoints.
...
`fixtures/p08-req-headers.jsonl` remain target-agnostic. Other gateways
...
Against the Wallarm API Gateway (arm64 or amd64 native — **not**
under qemu emulation, see below):

```
==> parity: gateway=wallarm profile=p08-req-headers target=http://localhost:9080
    fixture: fixtures/p08-req-headers.jsonl
...
On Apple Silicon, `docker pull --platform linux/amd64 the Wallarm API Gateway`
...
so a plain `docker pull the Wallarm API Gateway` (no `--platform`)
