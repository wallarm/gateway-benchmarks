#!/usr/bin/env bash
# gateways/wallarm/p08-req-body/setup.sh
#
# Bootstrap the p08-req-body policy profile via the Wallarm Admin API.
#
...
#   - The public image does not ship a dedicated `body_transform`
...
# trailing-slash strip from wallarm is harmless here — unlike p06/p07
# where strict `/headers`/`/response-headers` 404 on trailing slash).
SERVICE_NAME="bench-p08-anything"
...
# -----------------------------------------------------------------------------
say "wallarm/p08-req-body: bootstrap via ${ADMIN_URL}"
...
http_code=$(curl -sS -o /tmp/wallarm-p08.out -w '%{http_code}' \
...
say "wallarm/p08-req-body ready"
