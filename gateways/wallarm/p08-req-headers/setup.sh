#!/usr/bin/env bash
# gateways/wallarm/p08-req-headers/setup.sh
#
# Bootstrap the p08-req-headers policy profile via the Wallarm Admin API.
#
# Canonical policy (docs/POLICIES.md § p07):
#   add:    X-Bench-In: 1
#   remove: X-Forwarded-For
#
# Wallarm API Gateway implementation:
#   - The gateway does not yet ship a dedicated `header_transform`
...
#
#   - Base-path strip: the Wallarm gateway strips `base_path` and prepends
...
SERVICE_NAME="bench-p08-headers"
...
# -----------------------------------------------------------------------------
say "wallarm/p08-req-headers: bootstrap via ${ADMIN_URL}"
...
http_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
...
case "${http_code}" in
    ...
    *)       cat /tmp/wallarm-p07.out >&2
             fail "service create returned ${http_code}";;
esac

route_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
...
flow_body=$(jq -cn \
    ...
    '{
        request_flow: [{
            ...
            policy_name: "bench-p08-req-headers",
            config:      { code: $code }
        }]
    }')

flow_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
...
smoke=$(curl --max-time 5 -sS -H 'X-Forwarded-For: 1.2.3.4' "${DATA_URL}/headers")
...
say "wallarm/p08-req-headers ready"
