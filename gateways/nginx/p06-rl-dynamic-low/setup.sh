#!/usr/bin/env bash
# gateways/nginx/p06-rl-dynamic-low/setup.sh
#
# Post-up smoke for nginx/p06-rl-dynamic-low. nginx has no admin
...
say "nginx/p06-rl-dynamic-low: waiting for ${DATA_URL}"
for _ in $(seq 1 30); do
    if curl -sS -o /dev/null --max-time 2 \
            -H 'X-Real-IP: 10.0.0.1' "${DATA_URL}/anything" 2>/dev/null; then
...
say "nginx/p06-rl-dynamic-low ready"
