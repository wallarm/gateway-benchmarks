# `nginx / p06-req-headers` — compliance notes
...
* **No `X-Real-IP` pass-through** in this profile. p01/p03/p04/p05
  all send `X-Real-IP: $remote_addr` to the upstream; p06
  ...
  profile for p06 will drive through the same config, so this
...
The wallarm cell for p06 has the same behavioural contract but
implements it via `lua_runner` (`ctx.request.headers[...] = nil`),
because wallarm 0.2.0 has no first-class `header_transform`
policy. nginx's `proxy_set_header` path is strictly simpler and
