# `nginx / p04-rl-static` — compliance notes
...
| `1200 rps / 1 s → ≥ 150 (±50) × 429`           | `limit_req_zone $server_name zone=bench_p04:1m rate=1000r/s;` + `limit_req zone=bench_p04 burst=200 nodelay;` |
...
```nginx
limit_req_zone $server_name zone=bench_p04:1m rate=1000r/s;
...
* `zone=bench_p04:1m` — 1 MB shared memory. The module stores
  ...
  keeps the config diffable against p05 / p06 which *will* need the
  capacity.
