...
rewrites. Because the current build of the Wallarm gateway does not
yet support a catch-all `base_path: "/"`, we register one service per
prefix touched by the fixtures (`/anything`, `/bytes`, `/status`,
`/headers`, `/response-headers`). See [Deviations](#deviations) below.
...
| HTTP/1.1 only (upstream)               | default in the Wallarm gateway           | matches |
...
**What differs.** Wallarm API Gateway rejects `base_path: "/"` with
...
(private) required a non-empty suffix in the current build. The fix is tracked in
...
**Status.** Accepted; will be revisited once the fix lands in a
tagged Wallarm release.
...
Against the Wallarm API Gateway built from sources, parity is
**4/4 PASS**:
