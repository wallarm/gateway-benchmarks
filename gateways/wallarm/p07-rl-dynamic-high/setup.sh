#!/usr/bin/env bash
# gateways/wallarm/p07-rl-dynamic-high/setup.sh
#
# Bootstrap the p07-rl-dynamic-high policy profile via the Wallarm Admin API.
#
...
#
# Wallarm API Gateway implementation: same `ratelimit` + context-expression
# idiom as p05, only the `rate` parameter changes (10 → 100). See
# p06-rl-dynamic-low/NOTES.md for the rationale behind `scope: service`
# / `window_type: sliding`.
#
# Environment:
...
SERVICE_NAME="bench-p07-anything"
...
# -----------------------------------------------------------------------------
say "wallarm/p07-rl-dynamic-high: bootstrap via ${ADMIN_URL}"
...
http_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p06.out -w '%{http_code}' \
...
route_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p06.out -w '%{http_code}' \
...
flow_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p06.out -w '%{http_code}' \
...
smoke_code=$(curl --max-time 5 -s -o /tmp/wallarm-p06.out -w '%{http_code}' \
...
jq -c '{smoke:"ok", method:.method, url:.url}' /tmp/wallarm-p06.out >&2 || true

say "wallarm/p07-rl-dynamic-high ready"
