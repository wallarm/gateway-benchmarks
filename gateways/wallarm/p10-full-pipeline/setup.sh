#!/usr/bin/env bash
# gateways/wallarm/p10-full-pipeline/setup.sh
#
# Bootstrap the p10-full-pipeline policy profile via the Wallarm Admin API.
#
# Canonical order (docs/POLICIES.md § p10):
#   request:  jwt_validation -> ratelimit -> req-headers -> req-body
#   response: resp-body -> resp-headers
#
# Like p02-jwt, this script is dual-mode:
#   - public 0.2.0 images still report FEATURE-MISSING at runtime because
#     jwt_validation is absent from /policies;
#   - local overrides from current main can bind the full pipeline and run
#     the real parity fixture.
...
# -----------------------------------------------------------------------------
say "wallarm/p10-full-pipeline: bootstrap via ${ADMIN_URL}"
...
flow_body=$(jq -cn \
    ...
    '{
        request_flow: [
            ...
            {
                policy_id:   "ratelimit",
                policy_name: "bench-p10-rl",
                config: {
                    ratelimit_key: "bench-p10",
                    rate:          1000,
                    window:        1,
                    window_type:   "sliding",
                    scope:         "service"
            ...
            },
            {
                ...
                policy_name: "bench-p10-req-headers",
        ...
        response_flow: [
            {
                policy_id:   "lua_runner",
                policy_name: "bench-p10-resp-body",
                config:      { code: $lua_res_b }
            },
            {
                policy_id:   "lua_runner",
                policy_name: "bench-p10-resp-headers",
...
missing_code=$(curl -s -o /tmp/wallarm-p10.out -w '%{http_code}' \
    -X POST \
    -H 'Content-Type: application/json' \
    --data-binary '{"msg":"x"}' \
...
happy_code=$(curl -sS \
    -D /tmp/wallarm-p10.headers \
    -o /tmp/wallarm-p10.body \
    -w '%{http_code}' \
    -X POST \
    ...
    "${DATA_URL}/anything" || true)
[[ "${happy_code}" == "200" ]] \
    || { cat /tmp/wallarm-p10.body >&2
         fail "smoke: expected 200 with valid token, got ${happy_code}"; }

body=/tmp/wallarm-p10.body
x_bench_out="$(
    awk 'tolower($1) == "x-bench-out:" {gsub(/\r/, "", $2); print $2}' /tmp/wallarm-p10.headers \
    | sed -n '$p'
)"
...
say "wallarm/p10-full-pipeline ready"
