#!/usr/bin/env bash
# gateways/envoy/p01-vanilla/setup.sh
#
# Envoy has no runtime admin-API config surface we rely on: the whole
# profile is expressed in envoy.yaml, which docker-compose mounts
# read-only at container start. There is nothing to bootstrap — this
# script only proves that envoy came up and proxies the backend
# correctly before the parity attestation runs.
#
# Environment:
...
# -----------------------------------------------------------------------------
# 1. Wait for the data plane (parity-gateway.sh already polls this, but
#    we re-poll here so the smoke check below has meaningful timing.)
# -----------------------------------------------------------------------------
say "envoy/p01-vanilla: waiting for ${DATA_URL}"
...
bytes=$(curl -sS -o /dev/null -w '%{size_download}' "${DATA_URL}/bytes/1024" || true)
[[ "${bytes}" == "1024" ]] \
    || fail "smoke /bytes/1024: expected 1024 bytes, got ${bytes}"

say "envoy/p01-vanilla ready"
