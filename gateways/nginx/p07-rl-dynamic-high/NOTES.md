# `nginx / p07-rl-dynamic-high` — compliance notes
...
```nginx
limit_req_zone $http_x_real_ip zone=bench_p07:10m rate=100r/s;
limit_req_status 429;

server {
    location / {
        limit_req zone=bench_p07 burst=20 nodelay;
...
* **Zone = `10m`.** This is the only non-trivial difference from
  p05's `1m` zone: p06's POLICIES-mandated pool is **50 000
  ...
  the nginx column meets p06 spec, but only with explicit zone
  sizing (the 1 MB floor of p03/p05 would evict keys and distort
...
* **Same key + 429 handler + uniform settings as p05.** The only
  per-profile knobs are `rate=`, `zone=`, `burst=`.

## Uniform-settings audit

Identical to `p01-vanilla` / `p04-rl-static` / `p06-rl-dynamic-low`.
No deviations from `docs/GATEWAYS.md § Uniform settings`.

## Why no setup-time burst sanity

Same rationale as
[`p04-rl-static/NOTES.md § Why no standalone sanity`](../p04-rl-static/NOTES.md#why-no-standalone-sanity-burst-produces-429-in-setupsh):
