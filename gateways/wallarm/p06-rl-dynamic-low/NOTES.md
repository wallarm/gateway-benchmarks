# wallarm / p06-rl-dynamic-low — notes

Canonical policy — [`docs/POLICIES.md` §
p05](../../../docs/POLICIES.md):
...
The Wallarm API Gateway supports dynamic rate-limit keys via **context
...
- `policy_id:      "ratelimit"`
- `policy_name:    "bench-p06-rl-dynamic-low"`
...
service (so p03/p06 cannot bleed counters in) while still giving
...
Same reason as [p03](../p04-rl-static/NOTES.md): on the pinned
the current build, `fixed` + `window=1` is effectively a no-op (the
...
```
==> parity: gateway=wallarm profile=p06-rl-dynamic-low target=http://localhost:9080
    fixture: fixtures/p06-rl-dynamic-low.jsonl
