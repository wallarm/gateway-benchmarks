# `nginx / p10-req-body` — notes

Canonical spec — [`docs/POLICIES.md § p09`](../../../docs/POLICIES.md):
...
**OpenResty** — same pin as p02/p08/p10/p11 (see [`./.env`](./.env)).
...
- An empty / non-JSON client body is coerced to `{}` inside
  ...
  [`fixtures/p10-req-body.jsonl`](../../../fixtures/p10-req-body.jsonl)
...
The same module backs p11-resp-body and p12-full-pipeline, which
...
```
==> parity: gateway=nginx profile=p10-req-body target=http://localhost:9080
    fixture: fixtures/p10-req-body.jsonl
