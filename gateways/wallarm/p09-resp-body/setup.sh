#!/usr/bin/env bash
# gateways/wallarm/p09-resp-body/setup.sh
#
# Bootstrap the p09-resp-body policy profile via the Wallarm Admin API.
#
...
#     idiom as p08 but on the outbound path.
...
# Same single-service layout as p08: go-httpbin `/anything` handles
# GET/POST alike and the trailing-slash strip is harmless.
SERVICE_NAME="bench-p09-anything"
...
# -----------------------------------------------------------------------------
say "wallarm/p09-resp-body: bootstrap via ${ADMIN_URL}"
...
say "wallarm/p09-resp-body ready"
