...
http_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p10.out -w '%{http_code}' \
...
route_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p10.out -w '%{http_code}' \
...
flow_code=$(curl --max-time 5 -sS -o /tmp/wallarm-p10.out -w '%{http_code}' \
...
smoke=$(curl --max-time 5 -sS "${DATA_URL}/anything")
