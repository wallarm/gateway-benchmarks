#!/usr/bin/env bash
# gateways/wallarm/p07-resp-headers/setup.sh
#
# Bootstrap the p07-resp-headers policy profile via the Wallarm Admin API.
#
...
#     idiom as p06 (see that profile's NOTES.md for the why).
#   - Two services (`bench-p07-rh` and `bench-p07-get`) because the
...
# -----------------------------------------------------------------------------
say "wallarm/p07-resp-headers: bootstrap via ${ADMIN_URL}"
...
create_service_with_lua_resp() {
    ...
    local svc_code
    svc_code=$(curl -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
    ...
    local route_code
    route_code=$(curl -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
    ...
    local flow_code
    flow_code=$(curl -sS -o /tmp/wallarm-p07.out -w '%{http_code}' \
...
# -----------------------------------------------------------------------------
create_service_with_lua_resp \
    "bench-p07-rh"  "/response-headers" "${BACKEND_URL}/anything/response-headers"
create_service_with_lua_resp \
    "bench-p07-get" "/get"              "${BACKEND_URL}/anything/get"
...
say "wallarm/p07-resp-headers ready"
