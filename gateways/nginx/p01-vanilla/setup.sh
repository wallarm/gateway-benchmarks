#!/usr/bin/env bash
# gateways/nginx/p01-vanilla/setup.sh
#
# nginx has no admin API: the whole profile is expressed in
# nginx.conf, which docker-compose mounts read-only at container
# start. There is nothing to bootstrap — this script only proves that
# nginx came up and proxies the backend correctly before the parity
# attestation runs.
#
# Environment:
...
# -----------------------------------------------------------------------------
say "nginx/p01-vanilla: waiting for ${DATA_URL}"
