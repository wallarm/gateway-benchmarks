# wallarm / p07-resp-headers — notes
...
bound to the service's **`response_flow`** (same idiom as p06, but on
the response side).
...
`fixtures/p07-resp-headers.jsonl` exercises two client-facing paths
(`/response-headers` and `/get`). Due to the same base-path strip
quirk documented in `gateways/wallarm/p06-req-headers/NOTES.md`, we
...
does, and we cannot reach that one through wallarm 0.2.0 without
...
Against native `wallarm/api-gateway:0.2.0` (see
[`p06-req-headers/NOTES.md`](../p06-req-headers/NOTES.md) for the
qemu-segfault gotcha on x86-under-arm):

```
==> parity: gateway=wallarm profile=p07-resp-headers target=http://localhost:9080
    fixture: fixtures/p07-resp-headers.jsonl
